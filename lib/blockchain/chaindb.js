...
class ChainDB {
  ...
  async getCuckooEntryByHeight(height) {
    assert(typeof height === 'number');
    ...
    const state = this.state;
    const entry = await this.getCuckooEntryByHash(hash);
  ...
  async getCuckooEntryByHash(hash) {
    assert(Buffer.isBuffer(hash));
    ...
    const entry = ChainEntry.read(raw);

    this.cacheHash.get(entry.hash, entry);
  ...
  getCuckooEntry(block) {
    if (typeof block === 'number')
      return this.getCuckooEntryByHeight(block);
    return this.getCuckooEntryByHash(block);
  }
  ...
  getCuckooPrevious(entry) {
    return this.getCuckooEntryByHash(entry.prevBlock);
  }
  ...
  getCuckooTip() {
    return this.getCuckooEntryByHash(this.state.tip);
  }
