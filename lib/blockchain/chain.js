...
class Chain extends AsyncEmitter {
  ...
  async open() {
    ...
    const iso = await this.db.getCuckooTip();
    assert(iso);

    this.iso = iso;
    this.isoheight = iso.height;

    this.logger.info('Chain Height: %d', tip.height, iso.height);
  ...
  async verifyCuckooBlock(block) {
    const unlock = await this.locker.lock();
    try {
      return await this._verifyCuckooBlock(block);
    } finally {
      unlock();
    }
  }

  async _verifyCuckooBlock(block) {
    const flags = common.flags.DEFAULT_FLAGS & ~common.flags.VERIFY_CUCKOO;
    return this.verifyContext(block, this.iso, flags);
  }
  ...
  /**
   * Get previous entry (cuckoo).
   * @param {ChainEntry} entry
   * @returns {Promise} - Returns ChainEntry
   */

  getCuckooPrevious(entry) {
    return this.db.getCuckooPrevious(entry);
  }
  ...
  async getDeployments(time, prev) {
    ...
    const timestamp = await this.getMedianTime(prev, time);
  ...
  setDeploymentState(state) {
    ...
    if (!this.state.hasCuckooBlock() && state.hasCuckooBlock())
      this.logger.warning('Cuckoo hard-fork has been activated.')
  ...
  async _add(block, flags, id) {
    ...
    if (flags & common.flags.VERIFY_CUCKOO) {
      if (!block.verifyNewPOW())
        throw new VerifyError(block, 'invalid', 'high-hash', 50);
    }
  ...
  getCuckooEntryByHeight(height) {
    return this.db.getCuckooEntryByHeight(height);
  }
  ...
  /**
   * Get median of three last blocks based on a timestamp.
      ...
      if (pow.targetReset) {
        ...
        while (prev.height !== 0
          ...
          if (cache)
            ...
            prev = await this.getCuckooPrevious(prev);
  ...
  /**
   * Calculate the next target using legacy bitcoin difficulty adjustment +
   * emergency difficulty adjust (EDA).
   * @param {Number} time - Next block timestamp.
   * @param {ChainEntry} prev - Previous entry.
   * @returns {Promise} - returns Number
   * (target is in compact/mantissa form).
  ...
  async getCuckooTarget(time, prev) {
    const pow = this.network.pow;
  ...
  async getTarget(time, prev) {
    ...
    if (!prev) {
      ...
      if (prev.time >= this.network.block.cuckooBlock) {
        return this.getCuckooTarget(time, prev);
      }
...
class ChainOptions {
  ...
  fromOptions(options) {
    ...
    if (options.cuckooBlock != null) {
      assert(typeof options.cuckooBlock === 'boolean');
      this.cuckooBlock = otpions.cuckooBlock;
    }
...
class DeploymentState {
  ...
  /**
   * Test whether Cuckoo Fork is active.
   * @returns {Boolean}
   */

  hasCuckooBlock() {
    return this.cuckooBlock;
  }
}
